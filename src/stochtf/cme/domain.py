...
def from_rect(shape, slices=None, origin=None):
    ...
    indices = cme.util.indices_ext(shape, slices, origin)
...
def from_mapping(state_mapping):
    ...
    """
    states = from_iter(state_mapping.keys())
    values = numpy.array(list(state_mapping.values()))
