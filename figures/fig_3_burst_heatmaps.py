"""
Burst frequency and burst size across the (k_on,s, k_on,n) plane, HD vs M.

Definitions used throughout (both models, exactly):

    tau_off = 1/(alpha_s + alpha_n)          identical in HD and M
    f       = 1/(tau_on + tau_off)           bursts per unit time
    b       = k_y * tau_on                   transcripts per burst
    <y>     = b f / gamma                    mean expression

M  : exclusive three-state star S <-> 0 <-> N.  The ON set has no internal
     transitions, so tau_on = phi_s/beta_s + phi_n/beta_n, which depends on
     binding only through phi_s = alpha_s/(alpha_s+alpha_n).  Consequently

         k_y/beta_n  <=  b  <=  k_y/beta_s          BOUNDED

HD : four-state square, independent sites, OR emission.  State 11 gives the
     ON set internal structure, tau_on = tau_off (1 - q_s q_n)/(q_s q_n) with
     q_j = beta_j/(alpha_j + beta_j), and b is unbounded above.

Both models obey f*b < k_y, i.e. <y> < k_y/gamma: mean expression cannot
exceed the constitutive rate.  M's attainable set in the (f, b) plane is
therefore the horizontal strip b in [k_y/beta_n, k_y/beta_s] beneath that
hyperbola -- a STRICT SUBSET of HD's, which fills the whole region.  Any
(f, b) compatible with M is also compatible with HD, so burst-parameter
inference can rule M out but can never favour it over HD.  Panel (h) is that
statement drawn.

Burst kinetics are computed here, not taken from stochtf.analytical:
heterodimer.t_on() routes through tbound(), which tests/test_analytical.py
marks as a strict xfail, and whose error reaches 30% at these rates.  The
renewal-reward identity used below closes by construction and was checked
against the phase-type MFPT to 4e-11.
"""
import os
...
XL = r"$k_{on,s}[\mathrm{SOX2}]=\alpha_s$  " + RATE
YL = r"$k_{on,n}[\mathrm{NANOG}]=\alpha_n$  " + RATE
...
def heat(a, Z, title, cmap, norm, cb_label, levels=None, lev_c="w",
    ...
    if note:
        a.text(0.96, 0.05, note, transform=a.transAxes, ha="right",
               va="bottom", fontsize=5.9, color="w",
               bbox=dict(fc="k", ec="none", alpha=.40, pad=1.6))
...
heat(ax[1, 0], B_M, r"M:  burst size $b=k_y\tau_{\rm on}$", "magma", bnorm,
     r"$b$ (transcripts)", [6, 8, 10, 12, 14, 16, 18, 19.5],
...
heat(ax[1, 1], B_H, r"HD:  burst size $b=k_y\tau_{\rm on}$", "magma", bnorm,
     r"$b$ (transcripts)", [20, 100, 1e3, 1e4, 1e5],
...
a.set_xlabel(r"burst frequency $f$  " + RATE, fontsize=7.5)
