...
class ModelState:
    ...
    def __init__(
        ...
        chromatin_potential_dimer_partners = np.abs(
            chromatin_lattice_indicies[:, None] - chromatin_lattice_indicies[None, :]
        )
        self.dist_weighted_dimer_partners = np.exp(
            -chromatin_potential_dimer_partners / 1
        )
        ...
        self.bivalent_transition_matrix = np.zeros_like(
            self.dist_weighted_dimer_partners
        )
        self.total_dimer_weight_symmetric = 0.0
    ...
    def update_site_weights(self, site):
        ...
        if self.chromatin_all_undimered_monomers[site]:
            ...
            new_dimer_row[valid_mask] = self.dist_weighted_dimer_partners[
                site, valid_mask
            ]
        ...
        if self.chromatin_partner_state[site] in (-2, -3):
            ...
            new_tether_row[vacant_mask] = self.dist_weighted_dimer_partners[
                site, vacant_mask
            ]
        ...
        if self.chromatin_site_is_vacant[site]:
            ...
            new_tether_col[tether_mask] = self.dist_weighted_dimer_partners[
                tether_mask, site
            ]
    ...
    def set_site_state(
        ...
        # 4. Handle Partner Connections (Dangling or Bivalent)
...
class ModelLogger:
    ...
    def record_reaction(
        ...
        reaction_name = (
            config.REACTION_NAMES[reaction_index]
            if reaction_index < len(config.REACTION_NAMES)
...
class ModelReactions:
    ...
    def _update_site_times(
        ...
        reaction_name = (
            config.REACTION_NAMES[reaction]
            if reaction < len(config.REACTION_NAMES)
        ...
        if reaction_index == 2:  # SOX2-mediated binding
            ...
            if rand_val < bulk_s + bulk_ns:
                ...
                if rand_val < bulk_s:
                    ...
                    self.state.set_site_state(
                        primary_site, is_vacant=False, tf_type=tf_type, partner_state=-3
                    )  # NANOG dangles
            ...
            if rand_val < bulk_n + bulk_nn + bulk_ns:
                ...
                if rand_val < bulk_n:
                    ...
                    self.state.set_site_state(
                        primary_site, is_vacant=False, tf_type=tf_type, partner_state=-2
                    )  # SOX2 dangles
                    c[SPECIES_MAP["nanog_sox2_dimer_free"]] -= 1
                    c[SPECIES_MAP["nanog_sox2_dimer_single_bound"]] += 1
    ...
    def _handle_tf_unbinding(self, current_t, reaction_index):
        ...
        bound_indices = (
            list(self.state.bound_sox2_sites)
            if tf_type == 1
            else list(self.state.bound_nanog_sites)
        )
        ...
        if partner_state >= 0:
            ...
            if self._is_heterodimer(tf_type, partner_tf_type):
                ...
                self.state.species_counts[
                    SPECIES_MAP["nanog_sox2_dimer_single_bound"]
                ] += 1
            else:
                self.state.species_counts[SPECIES_MAP["nanog_nanog_dimer_bound"]] -= 1
                self.state.species_counts[
                    SPECIES_MAP["nanog_nanog_dimer_single_bound"]
                ] += 1
    ...
    def _execute_tethered_binding_logic(
        ...
        self.state.set_site_state(
            target_vacant_site, is_vacant=False, tf_type=dangling_type
        )
    ...
    def _execute_site_bulk_dimerise(self, current_t, reaction_index):
        ...
        if rand_val < sn_dimers and len(self.state.undimered_sox2_sites) > 0:
            ...
            and len(self.state.undimered_nanog_sites) > 0
        ):
...
class ModelCall:
    ...
    def __init__(
        ...
        self.stoichiometry_matrix = np.zeros(
            (len(config.SPECIES_NAMES), len(config.REACTION_NAMES)), dtype=np.int32
        )
        ...
        propensities = np.array(
            [
                ...
                * (
                ...
                self.rates.get("k_unbind_s", 0.0)
                * len(self.state.bound_sites.get(1, set())),
                self.rates.get("k_unbind_n", 0.0)
                * len(self.state.bound_sites.get(2, set())),
                self.rates.get("k_prod_m", 0.0) if promoter_on else 0.0,
                self.rates.get("k_deg_m", 0.0) * c[SPECIES_MAP.get("mRNA", 10)],
                self.rates.get("k_dimerise", 0.0)
                * (self.state.total_dimer_weight_symmetric / 2.0),
                self.rates.get("k_dimerise", 0.0) * (bulk_nn + bulk_sn),
                self.rates.get("k_dissociate", 0.0)
                * (len(self.state.dimered_dimer_sites) / 2.0),
                self.rates.get("k_dissociate", 0.0) * (ns_free + nn_free),
                self.rates.get("k_dimerise", 0.0) * site_bulk,
                self.rates.get("k_dissociate", 0.0)
                * (sum(self.state.dangling_counts.values())),
            ]
        )
